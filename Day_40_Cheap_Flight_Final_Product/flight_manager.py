...
class FlightManager:
    def __init__(self, city : str, iata_code : str, lower_price : str, customer_mails : list):
        self.city = city
        self.iata_code = iata_code
        self.lower_price = lower_price
        self.customer_mails = customer_mails
    ...
    def send_mail(self, departure_date, price):
        ...
        with smtplib.SMTP_SSL('smtp.gmail.com', 465, context=context) as connection:
            ...
            for customer in self.customer_mails:
                try:
                    connection.sendmail(MY_EMAIL, customer, msg=f"Subject:CHEAP FLIGHT AVAILABLE\n\nA cheap flight for {self.city} available on {departure_date} at price {price}")
                except smtplib.SMTPRecipientsRefused:
                    print(f"Invalid Email Address: {customer}")
                    self.customer_mails.remove(customer)
                    continue
